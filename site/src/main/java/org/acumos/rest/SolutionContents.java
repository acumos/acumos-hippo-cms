...
public class SolutionContents extends BaseRestResource {
	...
	public SolutionDescriptionContent createSolDescription(@Context HttpServletRequest request,
		...
		createSolDescription(workspace, description, requestContext, wpm);
		return description;

	}

	public void createSolDescription(String workspace, SolutionDescriptionContent description,
		...
		try {
			...
			/*
			 * No need to publish the Item if we can retrieve it without
			 * publishing wpm.setWorkflowCallbackHandler(new
			 * BaseWorkflowCallbackHandler<DocumentWorkflow>() { public void
			 * processWorkflow(DocumentWorkflow wf) throws Exception {
			 * wf.publish(); } });
			 */
			...
			if (descriptionBean == null) {
				...
				// log.debug("Found Description Bean : " +
				// descriptionBean.getCanonicalPath());
			}
	...
	public SolutionDescriptionContent getSolDescription(@Context HttpServletRequest request,
		...
		HstRequestContext requestContext = getRequestContext(request);
		return createSolDescription(requestContext, solutionId, revisionId, workspace);
	}

	public SolutionDescriptionContent createSolDescription(HstRequestContext requestContext, String solutionId,
			String revisionId, String workspace) {
	...
	public SolutionDescriptionContent getSolTermsCondition(@Context HttpServletRequest request,
		...
		HstRequestContext requestContext = getRequestContext(request);
		return getSolTermsCondition(requestContext);
	}

	public SolutionDescriptionContent getSolTermsCondition(HstRequestContext requestContext) {
		SolutionDescriptionContent description = new SolutionDescriptionContent();
	...
	public SolutionDescriptionContent getSolModelerResourceContent(@Context HttpServletRequest request,
		...
		HstRequestContext requestContext = getRequestContext(request);
		return getSolModelerResourceContent(requestContext, modelName);
	}

	public SolutionDescriptionContent getSolModelerResourceContent(HstRequestContext requestContext,
			String modelName) {
		SolutionDescriptionContent description = new SolutionDescriptionContent();
	...
	public SolutionDescriptionContent getSolDescriptionContent(@Context HttpServletRequest request,
			...
			@QueryParam("name") String contentName) {
		HstRequestContext requestContext = getRequestContext(request);
		return getSolDescriptionContent(requestContext, contentPath, contentName);
	}

	public SolutionDescriptionContent getSolDescriptionContent(HstRequestContext requestContext, String contentPath,
			String contentName) {
		SolutionDescriptionContent description = new SolutionDescriptionContent();
	...
	public JsonResponse<String> saveImage(@Context HttpServletRequest request, @Context HttpServletResponse response,
		...
		if (!imgHandle.hasNode(name)) {
			 ...
			 * BufferedImage.TYPE_INT_RGB); Graphics2D g2d =
			 * imageFile.createGraphics(); g2d.setPaint(Color.blue);
	...
	public JsonResponse<List<String>> getSolutionImageName(@Context HttpServletRequest request,
			...
			throws PathNotFoundException, LoginException, RepositoryException, IOException {
		HstRequestContext requestContext = getRequestContext(request);
		return getSolutionImageName(response, requestContext, solutionId);
	}

	public JsonResponse<List<String>> getSolutionImageName(HttpServletResponse response,
			HstRequestContext requestContext, String solutionId)
			throws PathNotFoundException, LoginException, RepositoryException, IOException {
	...
	public JsonResponse<String> saveAssets(@Context HttpServletRequest request, @Context HttpServletResponse response,
		...
		HstRequestContext requestContext = getRequestContext(request);
		return saveAssets(response, requestContext, solutionId, revisionId, path, attachment);
	}

	public JsonResponse<String> saveAssets(HttpServletResponse response, HstRequestContext requestContext,
			String solutionId, String revisionId, String path, Attachment attachment)
			throws PathNotFoundException, LoginException, RepositoryException, IOException {
		...
		// Count the total path attached to the node. Do not allow more that 7
		// files per solutionId
	...
	public JsonResponse<List<String>> copySolutionDocuments(@Context HttpServletRequest request,
		...
		JsonResponse<List<String>> test = copySolutionDocuments(requestContext, response, solutionId, revisionId, fromRevisionId, path);
		
		return test;
		
	}

	public JsonResponse<List<String>> copySolutionDocuments(HstRequestContext requestContext, HttpServletResponse response, String solutionId,
			String revisionId, String fromRevisionId, String path)throws PathNotFoundException, LoginException, RepositoryException, IOException {
		...
		// Get the next source node and pass it to create in destination.
		// Remaining all nodes will be created in copy nodes
	...
	public JsonResponse<String> saveGlobalImage(@Context HttpServletRequest request,
		...
		JsonResponse<String> test = saveGlobalImage(requestContext, response, path, attachment);
		
		return test;
	}

	public JsonResponse<String> saveGlobalImage(HstRequestContext requestContext, HttpServletResponse response, 
			String path, Attachment attachment)throws PathNotFoundException, LoginException, RepositoryException, IOException {
	...
	public JsonResponse<List<String>> getLogoImageName(@Context HttpServletRequest request,
		...
		JsonResponse<List<String>> test = getLogoImageName(response, requestContext, path);
		
		return test;
	}

	public JsonResponse<List<String>> getLogoImageName(HttpServletResponse response, HstRequestContext requestContext, 
			String path)throws PathNotFoundException, LoginException, RepositoryException, IOException {
	...
	private Node getOrCreatePathNodeForAsset(Session session, String solutionId, String revisionId, String path) {
		...
		try {
			 ...
			 * {"new-file-folder"}; pathNode.setProperty("hippostd:foldertype",
			 * foldertype); String[] gallerytype =
			 * {"hippogallery:exampleAssetSet"};
			 * pathNode.setProperty("hippostd:gallerytype", gallerytype); } else
			 * { pathNode = revisionIdFolder.getNode(path); }
			 */
